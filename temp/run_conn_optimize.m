...
xlsFile    = fullfile(tempDir, 'compare_QC_scores_optimized.xlsx');
...
% If template has no aroma entry, collapse the AROMA grid to {0}
HAS_AROMA = any(strcmp(orig_preproc.confounds.names, 'aroma'));
if ~HAS_AROMA
    log_msg(logfid, 'Template has no aroma confound -> AROMA search set to {0}');
    AROMA_ALL    = 0;
    AROMA_COARSE = 0;
end
